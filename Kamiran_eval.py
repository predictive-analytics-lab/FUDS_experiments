...
for state in state_list_short:
    ...
    train_data: em.DataTuple = em.acs_income(root=root_dir, year=2018, states=[state], horizon=1).load()
    ...
    for i in range(10): #10-fold cross validation, save values for each fold.
        ...
        metrics = [em.Accuracy()]
        per_sens = [em.Accuracy(), em.ProbPos(),  em.TPR(), em.FNR(), em.FPR(), em.PPV(), em.NPV()]
